...
class HarnessTest < Minitest::Test
  ...
  def test_perform_health_check_full_check_makes_api_call
    harness = AgentHarness::Harness.new(
      agent_id: "test-health-001",
      input: @input,
      output: @output,
      llm: @llm,
      config: { health_check_ttl: 300 }
    )

    # Full check should call available? without lightweight flag
    harness.send(:perform_health_check, full_check: true)

    assert_equal 1, @llm.available_call_count
    assert_equal 0, @llm.available_lightweight_calls
  end

  def test_perform_health_check_lightweight_uses_cache
    harness = AgentHarness::Harness.new(
      agent_id: "test-health-002",
      input: @input,
      output: @output,
      llm: @llm,
      config: { health_check_ttl: 300 }
    ...
    # First call (full check) - should make API call
    harness.send(:perform_health_check, full_check: true)
    assert_equal 1, @llm.available_call_count

    # Second call (lightweight) - should use cache, no additional call
    harness.send(:perform_health_check, full_check: false)
    assert_equal 1, @llm.available_call_count # Still 1, no new call
  end

  def test_health_check_cache_expires
    harness = AgentHarness::Harness.new(
      agent_id: "test-health-003",
      input: @input,
      output: @output,
      llm: @llm,
      config: { health_check_ttl: 1 } # 1 second TTL for testing
    )
    ...
    # Second call should make new API call because cache expired
    harness.send(:perform_health_check, full_check: false)
    assert_equal 2, @llm.available_call_count
  end

  def test_health_check_uses_lightweight_for_periodic
    harness = AgentHarness::Harness.new(
      agent_id: "test-health-004",
      input: @input,
      output: @output,
      llm: @llm,
      config: { health_check_ttl: 300 }
    ...
    # Periodic health checks (full_check: false) should use lightweight mode
    harness.send(:perform_health_check, full_check: false)

    assert_equal 1, @llm.available_call_count
    assert_equal 1, @llm.available_lightweight_calls
  end
end
